    ...
    try {
      ...
      if (authError) {
        if (authError.message.includes('rate limit')) {
          throw new Error('Email rate limit exceeded. Please try again later or use Google Sign-in.')
        }
        throw authError
      }
      ...
      if (role === 'patient') {
        await supabase.from('patient_profiles').insert({
          user_id: (await supabase.from('users').select('id').eq('auth_id', authData.user.id).single()).data?.id,
      ...
      if (authData.session === null) {
        ...
        setStep(4) // Move to a new step that shows the success message
      } else {
        // Logged in directly (email confirmation is off)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FDFDF9] font-sans p-4">
      <div className="w-full max-w-4xl text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-[#144533] mb-3">Samriti</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
              ...
              >
                ...
                <p className="text-gray-500 font-medium text-center">
                  Starting daily tasks, play games...
                </p>
              </button>
              ...
              >
                ...
                <p className="text-gray-500 font-medium text-center">
                  Monitor progress, set timings, alerts...
                </p>
              </button>
            </div>
        ...
        {step === 2 && (
          <div className="max-w-md mx-auto bg-white p-8 rounded-3xl shadow-sm border border-gray-100 text-left">
            ...
            <form onSubmit={(e) => { e.preventDefault(); setStep(3); }} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-[#144533] text-white font-bold rounded-xl py-3.5 mt-4 hover:bg-[#1B4D3E] transition-colors">
                Continue
              </button>
              ...
              <button 
                type="button"
                onClick={async () => {
                  localStorage.setItem('signup_role', role || 'patient');
                  await supabase.auth.signInWithOAuth({ provider: 'google' });
                }}
                className="w-full bg-white border border-gray-200 text-gray-700 font-bold rounded-xl py-3.5 flex items-center justify-center gap-3 hover:bg-gray-50 transition-colors shadow-sm"
              >
