...
// 'Key': { en: '...', hi: '...', mr: '...', gu: '...', bn: '...', ta: '...', te: '...', as: '', kha: '', lus: '', nag: '' },
// We need to replace the empty strings for as, kha, lus, nag with the translations from our JSON if available.
...
const newLines = lines.map(line => {
  if (line.includes("en:") && line.includes("as:")) {
    ...
    if (match) {
      // the first match might have escaped quotes, but let's try to match exact literal
      // It's easier to find the key in translations if we match the start of the line precisely.
      // E.g.   'Today': { ... }
      let key = match[1];
      // remove escaping for matching (e.g. Today\'s Tasks -> Today's Tasks)
      let rawKey = key.replace(/\\'/g, "'");
      
      if (translations[rawKey]) {
        const trans = translations[rawKey];
        // replace as: '', kha: '', lus: '', nag: '' with the translations
        let newLine = line;
        newLine = newLine.replace(/as: '.*?'/, `as: '${trans[0]}'`);
        newLine = newLine.replace(/kha: '.*?'/, `kha: '${trans[1]}'`);
        newLine = newLine.replace(/lus: '.*?'/, `lus: '${trans[2]}'`);
        newLine = newLine.replace(/nag: '.*?'/, `nag: '${trans[3]}'`);
        return newLine;
      }
    }
  }
